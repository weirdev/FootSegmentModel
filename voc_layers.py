...
class SBDDSegDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        # config
        params = eval(self.param_str)
        self.sbdd_dir = params['sbdd_dir']exposed
        self.split = params['split']
        self.mean = np.array(params['mean'])
        self.random = params.get('randomize', True)
        self.seed = params.get('seed', None)
...
class Deploy_VOCSegDataLayer(caffe.Layer):
    """
    Load (input image, label image) pairs from PASCAL VOC
    one-at-a-time while reshaping the net to preserve dimensions.
    ...
    def setup(self, bottom, top):
        ...
        # one top: data
        if len(top) != 1:
            raise Exception("Need to define one top: data.")
        # data layers have no bottoms
        if len(bottom) != 0:
            raise Exception("Do not define a bottom.")
        ...
        #self.indices = open(split_f, 'r').read().splitlines()
        self.idx = 0
        with open("deploy.txt") as deployfile:
            lines = deployfile.readlines()
    ...
    def load_image(self, idx):
        ...
        - transpose to channel x height x width order
        """
        im = Image.open(self.imgpaths[idx])
        in_ = np.array(im, dtype=np.float32)
        in_ = in_[:,:,::-1]
        in_ -= self.mean
        in_ = in_.transpose((2,0,1))
